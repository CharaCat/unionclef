...
public class MovementParkour extends Movement {
    ...
    private MovementParkour(IBaritone baritone, BetterBlockPos src, int dist, Direction dir, boolean ascend) {
        super(baritone, src, src.add(dir, dist).above(ascend ? 1 : 0), EMPTY, src.add(dir, dist).below(ascend ? 0 : 1));
        this.direction = dir;
        this.dist = dist;
        this.ascend = ascend;
    }
    ...
    protected Set<BetterBlockPos> calculateValidPositions() {
        ...
        for (int i = 0; i <= dist; i++) {
            for (int y = 0; y < 2; y++) {
                set.add(src.add(direction, i).above(y));
            }
        }
        return set;
    }
    ...
    public MovementState updateState(MovementState state) {
        ...
        if (ctx.playerFeet().equals(dest)) {
            ...
            } else if (!ctx.playerFeet().equals(dest.add(direction, -1))) {
                state.setInput(Input.SPRINT, false);
                if (ctx.playerFeet().equals(src.add(direction, -1))) {
                    MovementHelper.moveTowards(ctx, state, src);
                } else {
                    MovementHelper.moveTowards(ctx, state, src.add(direction, -1));
                }
            }
        }
        return state;
