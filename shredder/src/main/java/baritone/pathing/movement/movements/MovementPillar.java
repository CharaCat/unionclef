...
public class MovementPillar extends Movement {
    ...
    public MovementState updateState(MovementState state) {
        ...
        Rotation rotation = RotationUtils.calcRotationFromVec3d(ctx.playerHead(),
                ...
                ctx.playerRotations());
        if (!ladder) {
            state.setTarget(new MovementState.MovementTarget(ctx.playerRotations().withPitch(rotation.getXRot()), true));
        }

        boolean blockIsThere = MovementHelper.canWalkOn(ctx, src) || ladder;
        if (ladder) {
            BlockPos against = vine ? getAgainst(new CalculationContext(baritone), src) : src.add(fromDown.getValue(LadderBlock.FACING).getOpposite());
            if (against == null) {
