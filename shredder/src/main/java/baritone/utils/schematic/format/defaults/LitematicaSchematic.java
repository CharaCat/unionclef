...
public final class LitematicaSchematic extends StaticSchematic {
    ...
    private static String[] getRegions(CompoundTag nbt) {
        return nbt.getCompoundOrEmpty("Regions").keySet().toArray(new String[0]);
    }
    ...
    private static BlockState[] getBlockList(ListTag blockStatePalette) {
        ...
        for (int i = 0; i < blockStatePalette.size(); i++) {
            Block block = BuiltInRegistries.BLOCK.getValue(Identifier.parse(((CompoundTag) blockStatePalette.get(i)).getString("Name").orElse("")));
    ...
    private void fillInSchematic() {
        for (String subReg : getRegions(nbt)) {
            ListTag usedBlockTypes = nbt.getCompoundOrEmpty("Regions").getCompoundOrEmpty(subReg).getListOrEmpty("BlockStatePalette");
